  ...
  * @version V1.0
  * @date    2015-xx-xx
  * @brief   spi flash 底层应用函数bsp ,文件系统版本
  ******************************************************************************
  * @attention
  *
...
DSTATUS TM_FATFS_FLASH_SPI_disk_initialize(void)
{
 ...
 /**
  * @brief  文件系统移植接口
	*					返回SPI FLASH的状态，供文件系统查询状态接口调用
  * @param 	无
  * @retval FLASH正常：STA_NOINIT位 = 0 ，FLASH不正常：STA_NOINIT =1
  */
DSTATUS TM_FATFS_FLASH_SPI_disk_status(void)
{
 ...
 /**
  * @brief  文件系统移植接口
	*					查询SPI FLASH的容量信息，供文件系统格式化接口调用，
	*					通过本函数分配了FLASH后10MB给文件系统。
  * @param 	
	*		@arg 	cmd：文件系统的查询命令，对不同命令应返回不同信息。
	*		@arg	buff：根据命令给文件系统命令存储返回参数的指针。
  * @retval 直接返回RES_OK;
  */
DRESULT TM_FATFS_FLASH_SPI_disk_ioctl(BYTE cmd, char *buff)
{
	...
	switch (cmd) {
		case GET_SECTOR_COUNT:
			*(DWORD * )buff = 2560;		//sector数量   2560*4096/1024/1024=10(MB)
		break;
 ...
 /**
  * @brief  文件系统移植接口
	*					SPI FLASH读取数据，供文件系统读数据接口调用
  * @param 	
	*		@arg 	buff：存储读取数据的buff
	*		@arg	sector：文件系统要求读取的sector
	*		@arg 	conut:要读取的sector数
  * @retval 直接返回RES_OK;
  */
DRESULT TM_FATFS_FLASH_SPI_disk_read(BYTE *buff, DWORD sector, UINT count)
{
	...
	sector+=1536;//扇区偏移6MB，外部Flash文件系统空间放在外部Flash后面10MB空间
 ...
 /**
  * @brief  文件系统移植接口
	*					SPI FLASH写入数据，供文件系统写数据接口调用
  * @param 	
	*		@arg 	buff：存储写入数据的buff
	*		@arg	sector：文件系统要求写入的sector
	*		@arg 	conut:要写入的sector数，经测试文件系统每次count都为1，所以这里只写一次。
  * @retval 直接返回RES_OK;
  */
